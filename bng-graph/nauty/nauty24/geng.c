...
OUTPROC feature.
   ...
   geng can be made to call a procedure of your manufacture with each
   ...
   and n is the number of vertices.  Your procedure can be in a separate 
   file so long as it is linked with geng.  The global variables 
   nooutput, nautyformat, yformat and canonise (all type boolean) can 
   be used to test for the presence of the flags -u, -n, -y and -l, 
   respectively.  If -l is present, the group size and similar details
   can be found in the global variable nauty_stats.
...
**************************************************************************
    ...
    Author:   B. D. McKay, Sep 1991 and Jan 1997.
              Copyright  B. McKay (1991-1997).  All rights reserved.
...
static void
userautomprocb(int count, permutation *p, int *orbits,
              int numorbits, int stabvertex, int n)
/* form orbits on powerset of VG
   called by nauty;  operates on data[n] */
{
