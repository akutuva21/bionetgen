...
class TransformationMap:
	...
	def __init__(self,dictNames,rule_map):
		...
		self.t2p_context.update([  (dictTransformations[t],dictPatterns[p]) for r in dictRules for t in r.transformations for tr in r.transformations for p in tr.getLHS() if tr!=t ])
...
def getLevels(start,end,names,all_maps):
	...
	maps_by_tp = dict()
	for x, y in maps:
		maps_by_tp.setdefault(x, []).append(y)
	for tp in [x for x in names.tp.values() if x not in tp_levels]:
		possiblelevels = [p_levels[y] for y in maps_by_tp.get(tp, []) if y in p_levels]
	...
	maps2_by_irr = dict()
	for x, y in maps2:
		maps2_by_irr.setdefault(x, []).append(y)
	for irr in [x for x in names.irr.values() if x not in irr_levels]:
		possiblelevels = [p_levels[y] for y in maps2_by_irr.get(irr, []) if y in p_levels]
		if len(possiblelevels)>0:
			if max(p_levels.values()) in possiblelevels:
	...
	maps_by_p = dict()
	for x, y in maps:
		maps_by_p.setdefault(y, []).append(x)
	for p in [x for x in names.p.values() if x not in p_levels]:
		possiblelevels = [tp_levels[x] for x in maps_by_p.get(p, []) if x in tp_levels]
		if len(possiblelevels) > 0:
	...
	maps2_by_p = dict()
	for x, y in maps2:
		maps2_by_p.setdefault(y, []).append(x)
	for p in [x for x in names.p.values() if x not in p_levels]:
		possiblelevels = [irr_levels[x] for x in maps2_by_p.get(p, []) if x in irr_levels]
		if len(possiblelevels) > 0:
			p_levels[p] = min(possiblelevels)
...
def makeFlow(names,all_maps,start,end):
	...
	'''
	start  = resolveBondWildcards(start,names.p)
	end  = resolveBondWildcards(end,names.p)
	
	start = [names.p[x] for x in names.p if str(x) in start]
	end = [names.p[x] for x in names.p if str(x) in end]
