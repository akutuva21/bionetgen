...
class BipartiteServer:
    ...
    def bipartite(self, bbnglFile,returnType,center,context,product):
        ...
        try:
            ...
            with open(os.path.join(temp_dir, 'temp.xml.dot'), 'rb') as f:
                dot = f.read()
            with open(os.path.join(temp_dir, 'temp.xml.png'), 'rb') as f:
                png = f.read()
            if returnType == 'dot':
    ...
    def _bngl2xml(self,bnglFile):
        # SECURITY: Passed as a list to avoid command injection vulnerability
