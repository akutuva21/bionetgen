...
int main() {
    ...
    for (int iter = 0; iter < 1000000; ++iter) {
        // OPTIMIZED VERSION 3: string_view cache
        auto endIt = g_compartmentDimensions.end();
        std::string_view lastCompR;
        std::string_view lastCompP;
    ...
    std::cout << "Optimized (cache string_view): " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << " ms" << std::endl;
    std::cout << "Dummy: " << dummy << std::endl;
