...
std::string MacroBNGModel::pre_macr(const std::string& param_prefix) {
    ...
    while (!(line = get_line()).empty() && !abort_read) {
        ...
        if (std::regex_search(trimmed, m, re_begin)) {
            ...
            // --- molecule_types ---
            else if (std::regex_match(name, []() -> const std::regex& { static const std::regex re("^molecule[_ ]types$"); return re; }())) {
                // We store the type info via pre_species1 later; just count.
                std::cout << "Read " << block_dat.size() << " molecule types.\n";
            }
            // --- seed_species ---
            else if (std::regex_match(name, []() -> const std::regex& { static const std::regex re("^seed[_ ]species$"); return re; }())) {
            ...
            // --- reaction_rules ---
            else if (std::regex_match(name, []() -> const std::regex& { static const std::regex re("^reaction[_ ]rules$"); return re; }())) {
                int nerr_count = 0;
                for (auto& [entry, lno] : block_dat) {
                    ...
                    std::smatch rm;
                    if (std::regex_search(str1ing, rm, []() -> const std::regex& { static const std::regex re("^\\s*([^:].*)[:]\\s*"); return re; }())) {
                        nam1e = rm[1].str();
                        std::cerr << "ERROR 1002   rules  (user) name=" << nam1e << "\n";
                        return "ERROR 1002: named rules not supported";
                    } else if (std::regex_search(str1ing, rm, []() -> const std::regex& { static const std::regex re("^\\s*(\\d+)\\s+"); return re; }())) {
                        nam1e = rm[1].str();
            ...
            // --- observables ---
                ...
                for (auto& [entry, lno] : block_dat) {
                    ...
                    std::smatch rm;
                    if (std::regex_search(str1ing, rm, []() -> const std::regex& { static const std::regex re("^\\s*\\d+\\s+"); return re; }())) {
                        str1ing = rm.suffix().str();
                    }
        ...
        else if (std::regex_search(trimmed, m, []() -> const std::regex& { static const std::regex re("^([A-Za-z][^(]*)"); return re; }())) {
...
void MacroBNGModel::pre_species1(std::map<std::string, int>& nm_site,
    ...
    for (auto& ii : species2_) {
        ...
        {
            std::smatch m;
            if (std::regex_search(entry, m, []() -> const std::regex& { static const std::regex re("^\\s*([^:].*)[:]\\s*"); return re; }())) {
                name = m[1].str();
...
void MacroBNGModel::del_blank(const std::vector<std::string>& str,
    ...
    for (auto line : str) {
        ...
        std::smatch m;
        if (std::regex_search(line, m, []() -> const std::regex& { static const std::regex re("^\\d+?;"); return re; }())) {
            line = ";" + m.suffix().str();
        } else {
            line = ";" + line;
        }
