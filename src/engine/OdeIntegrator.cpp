...
bool hasWordBoundaryMatch(const std::string& text, const std::string& target, bool caseInsensitive = false) {
    ...
    while (it != text.end()) {
        if (!caseInsensitive) {
            std::size_t pos = text.find(target, std::distance(text.begin(), it));
            if (pos == std::string::npos) break;
            it = text.begin() + pos;
        } else {
            it = std::search(it, text.end(), target.begin(), target.end(), [](char c1, char c2) {
...
void OdeIntegrator::compile() {
    ...
    for (const auto& rxn : network_.reactions.all()) {
        ...
        if (!isFunctional && rateExpr.has_value()) {
            ...
            for (std::size_t i = 0; i + timeStr.length() <= rawRateLaw.length(); ++i) {
                bool match = true;
                for (std::size_t j = 0; j < timeStr.length(); ++j) {
                    if (std::tolower(static_cast<unsigned char>(rawRateLaw[i + j])) != timeStr[j]) {
                        match = false;
                        break;
                    }
                }
            ...
            if (!isFunctional) {
                for (const auto& func : model_.getFunctions()) {
                    if (hasWordBoundaryMatch(rawRateLaw, func.getName(), true)) {
                        isFunctional = true;
        ...
        if (!crxn.isFunctional) {
            ...
            for (const auto& func : model_.getFunctions()) {
                if (hasWordBoundaryMatch(rawRL, func.getName(), true)) {
                    crxn.isFunctional = true;
